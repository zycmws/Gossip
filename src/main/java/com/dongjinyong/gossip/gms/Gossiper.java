...
public class Gossiper implements IFailureDetectionEventListener, GossiperMBean {
  ...
  static final List<String> DEAD_STATES = Arrays
      .asList(VersionedValue.REMOVING_TOKEN, VersionedValue.REMOVED_TOKEN);
  ...
  /* live member set */
  private Set<InetSocketAddress> liveEndpoints = new ConcurrentSkipListSet<InetSocketAddress>(
      inetcomparator);
  ...
  private Set<InetSocketAddress> seeds = new ConcurrentSkipListSet<InetSocketAddress>(
      inetcomparator);
    ...
    public void run() {
      try {
        ...
        if (logger.isTraceEnabled()) {
          logger.trace(
              "My heartbeat is now " + endpointStateMap.get(FBUtilities.getBroadcastAddress())
  ...
  Message makeGossipDigestSynMessage(List<GossipDigest> gDigests) throws IOException {
    GossipDigestSynMessage gDigestMessage = new GossipDigestSynMessage(
        GossiperDescriptor.getClusterName(), gDigests);
    FastByteArrayOutputStream bos = new FastByteArrayOutputStream();
    DataOutputStream dos = new DataOutputStream(bos);
    GossipDigestSynMessage.serializer().serialize(gDigestMessage, dos);
    return new Message(FBUtilities.getBroadcastAddress(), MessageVerb.Verb.GOSSIP_DIGEST_SYN,
        bos.toByteArray());
  }
  ...
  Message makeGossipDigestAckMessage(GossipDigestAckMessage gDigestAckMessage) throws IOException {
    ...
    return new Message(FBUtilities.getBroadcastAddress(), MessageVerb.Verb.GOSSIP_DIGEST_ACK,
        bos.toByteArray());
  }
  ...
  Message makeGossipDigestAck2Message(GossipDigestAck2Message gDigestAck2Message)
    ...
    return new Message(FBUtilities.getBroadcastAddress(), MessageVerb.Verb.GOSSIP_DIGEST_ACK2,
        bos.toByteArray());
  }
  ...
  Message makeGossipShutdownMessage() throws IOException {
    ...
    return new Message(FBUtilities.getBroadcastAddress(), MessageVerb.Verb.GOSSIP_SHUTDOWN,
        bos.toByteArray());
  }
 ...
 */
  ...
  private void doStatusCheck() {
    ...
    for (InetSocketAddress endpoint : eps) {
      ...
      if (epState != null) {
        ...
        if (!isDeadState(epState) && !epState.isAlive() && !justRemovedEndpoints
            .containsKey(endpoint) && (duration > FatClientTimeout)) {
          logger.info("FatClient " + endpoint + " has been silent for " + FatClientTimeout
  ...
  EndpointState getStateForVersionBiggerThan(InetSocketAddress forEndpoint, int version) {
    ...
    if (epState != null) {
      ...
      if (localHbVersion > version) {
        ...
        if (logger.isTraceEnabled()) {
          logger.trace(
              "local heartbeat version " + localHbVersion + " greater than " + version + " for "
                  + forEndpoint);
        }
      }
      /* Accumulate all application states whose versions are greater than "version" variable */
      for (Entry<ApplicationState, VersionedValue> entry : epState
          .getApplicationStateMapEntrySet()) {
  ...
  private void markAlive(InetSocketAddress addr, EndpointState localState) {
    ...
    localState
        .updateTimestamp(); // prevents doStatusCheck from racing us and evicting if it was down > aVeryLongTime
  ...
  void applyStateLocally(Map<InetSocketAddress, EndpointState> epStateMap) {
    for (Entry<InetSocketAddress, EndpointState> entry : epStateMap.entrySet()) {
      ...
      if (localEpStatePtr != null) {
        ...
        if (logger.isTraceEnabled()) {
          logger.trace(ep + "local generation " + localGeneration + ", remote generation "
              + remoteGeneration);
        }

        if (remoteGeneration > localGeneration) {
          if (logger.isTraceEnabled()) {
            logger.trace("Updating heartbeat state generation to " + remoteGeneration + " from "
            ...
            == localGeneration) // generation has not changed, apply new states
        {
          ...
          if (remoteMaxVersion > localMaxVersion) {
            ...
            logger.trace(
                "Ignoring remote version " + remoteMaxVersion + " <= " + localMaxVersion + " for "
                    + ep);
          }
          if (!localEpStatePtr.isAlive() && !isDeadState(
              localEpStatePtr)) // unless of course, it was dead
          {
   ...
   * @param remoteState
   */
  private void applyNewStates(InetSocketAddress addr, EndpointState localState,
    ...
    if (logger.isTraceEnabled()) {
      logger.trace("Updating heartbeat state version to " + localState.getHeartBeatState()
          .getHeartBeatVersion() + " from " + oldVersion + " for " + addr + " ...");
    }

    for (Entry<ApplicationState, VersionedValue> remoteEntry : remoteState
        .getApplicationStateMapEntrySet()) {
  ...
  private void doNotifications(InetSocketAddress addr, ApplicationState state,
  ...
  private void sendAll(GossipDigest gDigest, Map<InetSocketAddress, EndpointState> deltaEpStateMap,
  ...
  */
  void examineGossiper(List<GossipDigest> gDigestList, List<GossipDigest> deltaGossipDigestList,
      Map<InetSocketAddress, EndpointState> deltaEpStateMap) {
  ...
  public void addSavedEndpoint(InetSocketAddress ep) {
    ...
    if (logger.isTraceEnabled()) {
      logger
          .trace("Adding saved endpoint " + ep + " " + epState.getHeartBeatState()
              .getGeneration());
    }
  }
...
//        logger.info("Removing token: " + token);
    logger.info("Sleeping for " + GossiperDescriptor.getRing_delay() + "ms to ensure " + endpoint
        + " does not change");
    try {
    ...
    if (epState.getHeartBeatState().getGeneration() != generation) {
      throw new RuntimeException(
          "Endpoint " + endpoint + " generation changed while trying to remove it");
    }
