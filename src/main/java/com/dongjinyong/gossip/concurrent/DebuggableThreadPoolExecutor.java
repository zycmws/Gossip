...
/**
 * This class encorporates some Executor best practices for Cassandra.  Most of the executors in the
 * system should use or extend this.  There are two main improvements over a vanilla TPE:
 *
 * - If a task throws an exception, the default uncaught exception handler will be invoked; if there
 * is no such handler, the exception will be logged. - MaximumPoolSize is not supported.  Here is
 * what that means (quoting TPE javadoc):
 *
 * If fewer than corePoolSize threads are running, the Executor always prefers adding a new thread
 * rather than queuing. If corePoolSize or more threads are running, the Executor always prefers
 * queuing a request rather than adding a new thread. If a request cannot be queued, a new thread is
 * created unless this would exceed maximumPoolSize, in which case, the task will be rejected.
 *
 * We don't want this last stage of creating new threads if the queue is full; it makes it
 * needlessly difficult to reason about the system's behavior.  In other words, if DebuggableTPE has
 * allocated our maximum number of (core) threads and the queue is full, we want the enqueuer to
 * block.  But to allow the number of threads to drop if a stage is less busy, core thread timeout
 * is enabled.
 */
public class DebuggableThreadPoolExecutor extends ThreadPoolExecutor {
  ...
  /**
   * Returns a ThreadPoolExecutor with a fixed number of threads. When all threads are actively
   * executing tasks, new tasks are queued. If (most) threads are expected to be idle most of the
   * time, prefer createWithMaxSize() instead.
   *
   ...
   */
  public static DebuggableThreadPoolExecutor createWithFixedPoolSize(String threadPoolName,
      int size) {
    return createWithMaximumPoolSize(threadPoolName, size, Integer.MAX_VALUE, TimeUnit.SECONDS);
  }

  /**
   * Returns a ThreadPoolExecutor with a fixed maximum number of threads, but whose threads are
   * terminated when idle for too long. When all threads are actively executing tasks, new tasks are
   * queued.
   *
   * @param threadPoolName the name of the threads created by this executor
   ...
   * @return the new DebuggableThreadPoolExecutor
   */
  public static DebuggableThreadPoolExecutor createWithMaximumPoolSize(String threadPoolName,
      int size, int keepAliveTime, TimeUnit unit) {
