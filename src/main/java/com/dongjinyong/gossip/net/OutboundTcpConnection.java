...
public class OutboundTcpConnection extends Thread {
  ...
  private static final Message SHUTDOWN_OUTBOUNDTCPCONNECTION_SENTINEL = new Message(
...
//                socket = poolReference.newSocket();
        socket = new Socket(poolReference.endPoint().getAddress(),
            poolReference.endPoint().getPort(), FBUtilities.getLocalAddress().getAddress(), 0);
  ...
  private void expireMessages() {
    while (true) {
      ...
      if (entry == null || entry.timestamp >= System.currentTimeMillis() - GossiperDescriptor
          .getRpcTimeout()) {
        break;
      }
