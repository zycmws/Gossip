...
public class DaemonTest2 {
  ...
  private void printEndpointStates() throws IOException {
    DebuggableScheduledThreadPoolExecutor executor = new DebuggableScheduledThreadPoolExecutor(
        "abc");
    executor.scheduleWithFixedDelay(new Runnable() {
                                      public void run() {

                                        Set<Entry<InetSocketAddress, EndpointState>> set = Gossiper.instance.getEndpointStates();
                                        for (Iterator<Entry<InetSocketAddress, EndpointState>> iterator = set.iterator();
                                            iterator.hasNext(); ) {
                                          Entry<InetSocketAddress, EndpointState> entry = iterator.next();
                                          System.out.println("key:" + entry.getKey() + ", value:" + entry.getValue());

                                          EndpointState endpoint = entry.getValue();
                                          for (Entry<ApplicationState, VersionedValue> entry2 : endpoint
                                              .getApplicationStateMapEntrySet()) {
                                            System.out.println(
                                                "VersionedValue----key:" + entry2.getKey() + ", value:" + entry2.getValue());
                                          }
                                        }
                                        System.out.println("=======================");
  ...
  public static void main(String[] args) throws IOException {
    ...
    service.getGossiper().register(new IApplicationStateStarting() {
      public void gossiperStarting() {
        service.getGossiper().addLocalApplicationState(ApplicationState.LOAD,
            VersionedValueFactory.instance.load(7.1));
        service.getGossiper().addLocalApplicationState(ApplicationState.WEIGHT,
            VersionedValueFactory.instance.weight(5));
